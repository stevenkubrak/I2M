...
presenter_id 						= cookie.CURRENT_USER_ID;
  ...
  try{
	...
	request.getAgentFormsDirectory		= request.initiation_cfc.getAgentFormsDirectory(user.id);
	reg 								= request.getAgentFormsDirectory.DIRECTORY;
	statusList							= "Y,N";
	request.available_users 			= request.initiation_cfc.selPeopleSubToPersonInRole(user.id);
 
//WriteDump(var=request.available_users,label="request.available_users",expand=false,abort=false);//Debugging only    
//WriteDump(var=cookie,label="cookies",expand=false,abort=false);//Debugging only
//WriteDump(var=request.getHQProgram,label="getHQProgram",expand=false,abort=false);//Debugging only
//WriteDump(var=application,label="application scope",expand=false,abort=false);//Debugging only
    ...
    <!--- <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <ul class="nav nav-tabs">
                ...
                <li role="presentation"><a href="/aps/i2m/views/complaints/inv_plan/dsp_complaint_investigative_plan.cfm">Inv Plan</a></li>
                <li role="presentation"><a href="##">Individuals/Institutions</a></li>   
                <li role="presentation"><a href="/aps/i2m/views/complaints/initiation/frm_complaint_staff.cfm">Staff</a></li>
...
<!---this list has to be generated based on whether the person is a sac, rac, agent, etc. --->
    <div class="form-group" id="agent_group">
        ...
        <cfloop query="request.available_users">
             <option value="#PERSON_ID#"#isSelected(PERSON_ID,cookie.current_user_id)# >#last_name#, #first_name#</option>
        </cfloop>
        </select>
     	</div>
    </div>
      ...
      <div class="form-group" id="hotline_group">
        ...
        <input type="text" id="is_hotline_number" name="is_hotline_number" value="" class="form-control">&nbsp;<i>YYYY-0000</i>
     </div> 
    </div>    
    ...
    <div class="form-group" id="initiation_desc_group">
		<div class="col-md-8">
        	<div class="hint"><br />You must use the initiation document template for this file. If you do not have a copy of this template, <a href="#application.base_path#/views/help/invforms/#reg#/1 Investigative Reports/Case-Complaint-Initiation.dotx" target="new">Click Here</a></div>        
        </div>
    </div>    
    
     <div class="form-group" id="file_group">
		<div class="col-md-8">
        <label class="control-label" for="file_name">Initiation Report Description</label>
        	<input type="text" name="file_desc" id="file_desc" value="CPI-#DateFormat(Now(), 'mm/dd/yyyy')#">
     	</div>
    </div>     
    
    <div class="form-group" id="file_group">
		<div class="col-md-8">
        <label class="control-label" for="file_name">Initiation Report</label>
        	<input type="file" name="file_name" accept="application/vnd.openxmlformats-officedocument.wordprocessingml.template">
     	</div>
    </div>
			...
			rules: {
				...
				messages: {
				   ...
				   case_agent:{
					...
					file_desc: {
					  required: "Please enter a file description"
					},
					file_name: {
					  required: "You must upload an Initiation document file to continue"
					},
					manchall: {
